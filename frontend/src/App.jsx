  ...
  const submitBooking = async () => {
    ...
    if (bookingFriends.trim() !== "") {
      ...
      if (friendsArray.length > 0) {
        ...
        if (!validUsers || validUsers.length !== friendsArray.length) return Swal.fire("Teman Tidak Valid!", "Pastikan semua nama teman sudah terdaftar.", "error");
      }
    }
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in">
          <div className="bg-slate-50 rounded-[2rem] shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-200">
            ...
            <div className="p-6 md:p-8 overflow-y-auto">
              ...
              {/* RIWAYAT TRANSAKSI DENGAN CARD LAYOUT KHUSUS HP */}
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden mb-10">
                <div className="p-4 md:p-8 border-b border-slate-100 flex flex-col gap-6">
                   <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      <div className="flex items-center gap-3">
                         ...
                         <h3 className="font-black text-xl sm:text-2xl text-slate-800 tracking-tight">Riwayat Transaksi</h3>
                      </div>
                      
                      <button onClick={async () => { 
                        const confirmResult = await Swal.fire({
                          title: 'Kosongkan Data?', text: "Semua riwayat booking akan dihapus permanen!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Ya, Bersihkan!'
                        });
                        if(confirmResult.isConfirmed) { 
                      ...
                      }} className="w-full sm:w-auto inline-flex justify-center items-center gap-2 bg-red-50 text-red-500 border border-red-200 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-colors cursor-pointer shadow-sm">
                        <span>🗑️</span> Kosongkan Data
                      </button>
                   </div>
                   
                   {/* KOTAK SCANNER QR CODE (MOBILE RESPONSIVE) */}
                   <div className="w-full bg-slate-800 p-5 rounded-2xl shadow-inner text-white flex flex-col sm:flex-row justify-between items-center gap-4">
                      <div className="text-center sm:text-left w-full sm:w-auto">
                         <h4 className="text-xs font-black uppercase tracking-widest text-blue-400">📷 Scan Tiket Pemain</h4>
                         <p className="text-[10px] text-slate-400 mt-1">Arahkan kamera ke QR Code pemain</p>
                      </div>
                      <button 
                         onClick={() => setShowCamera(!showCamera)} 
                         className={`w-full sm:w-auto px-6 py-3 rounded-xl font-black text-xs shadow-md transition-all cursor-pointer ${showCamera ? 'bg-red-500 text-white hover:bg-red-400' : 'bg-blue-600 text-white hover:bg-blue-500'}`}
                      >
                ...
                {/* TABEL RESPONSIVE MENGGUNAKAN CARD */}
                  ...
                  {revenueData.recentTransactions && revenueData.recentTransactions.length > 0 ? (
                     <div className="flex flex-col gap-4">
                       {revenueData.recentTransactions.map(tx => (
                         <div key={tx.id} className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4 hover:shadow-md transition-shadow">
                             
                             <div className="flex flex-col gap-1.5 w-full lg:w-1/3">
                                 <div className="flex items-center gap-2">
                                     <span className="font-black text-slate-800 text-sm sm:text-base">{tx.player_name}</span>
                                     {tx.is_public && <span className="bg-indigo-100 text-indigo-700 text-[10px] px-2 py-0.5 rounded-md font-black uppercase tracking-widest border border-indigo-200">Mabar</span>}
                                 </div>
                                 <p className="text-xs font-bold text-slate-500">📅 {tx.booking_date} | ⏰ {tx.start_time?.substring(0,5)}</p>
                                 <div className="text-[10px] font-bold text-slate-400 flex flex-col gap-0.5 mt-1">
                                    {tx.split_members && tx.split_members.length > 1 && <p>Patungan: <span className="text-slate-500">{tx.split_members.filter(m => m !== tx.player_name).join(', ')}</span></p>}
                                    {tx.joined_players && tx.joined_players.length > 0 && <p>Joiner: <span className="text-indigo-400">{tx.joined_players.join(', ')}</span></p>}
                                 </div>
                             </div>

                             <div className="flex flex-row lg:flex-col justify-between lg:justify-center items-center lg:items-start gap-2 bg-slate-50 lg:bg-transparent p-3 lg:p-0 rounded-xl border lg:border-none border-slate-100 w-full lg:w-1/3">
                                 {tx.additional_items && (tx.additional_items.racket > 0 || tx.additional_items.kok > 0) ? (
                                    <span className="bg-amber-50 border border-amber-100 text-amber-700 text-[10px] px-2 py-1.5 rounded-lg font-bold shadow-sm">
                                       🎒 Raket: {tx.additional_items.racket} | Kok: {tx.additional_items.kok}
                                    </span>
                                 ) : <span className="text-[10px] font-bold text-slate-300">Tanpa Sewa Alat</span>}
                             ...
                             <div className="w-full lg:w-auto pt-2 lg:pt-0 border-t lg:border-none border-slate-100">
                                 {tx.status !== 'checked-in' ? (
                                     <button onClick={() => verifyTicket(tx)} className="bg-blue-600 text-white w-full lg:w-40 py-3 lg:py-2.5 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-500 transition-colors shadow-md cursor-pointer">
                                        Verifikasi
                                     </button>
                                 ) : (
                                     <span className="bg-green-100 text-green-700 w-full lg:w-40 py-3 lg:py-2.5 rounded-xl text-xs font-black uppercase tracking-widest flex items-center justify-center border border-green-200">
                                        Masuk ✅
                                     </span>
                                 )}
                             </div>
                     ...
                     <div className="text-center py-12">
                       <p className="text-slate-400 font-bold bg-white inline-block px-6 py-3 rounded-2xl border border-slate-200 shadow-sm">Belum ada rekam jejak transaksi.</p>
                     </div>
                  )}
                </div>
              </div>
